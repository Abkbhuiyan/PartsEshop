<?php
	...
	if(isset($_SESSION['user'])){
		...
		try{
			$total = 0;
			$stmt = $conn->prepare("SELECT *, cart.id AS cartid,cart.product_id AS pID,cart.quantity AS cartquantity FROM cart LEFT JOIN products ON products.id=cart.product_id WHERE user_id=:user");
			...
			foreach($stmt as $row){
				...
				$output .= "
					<tr>
						...
						<td class='input-group'>
            				...
            				<input type='text' class='form-control' value='".$row['cartquantity']."' id='qty_".$row['cartid']."'>
				...
				";
				$_SESSION['cartquantity']=$row['cartquantity'];
				$_SESSION['productId']=$row['pID'];
				$_SESSION['productquantity']=$row['quantity'];
